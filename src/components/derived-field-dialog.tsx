...
import { Search, Close, ChevronRight, ChevronDown } from "@carbon/icons-react";
...
export function DerivedFieldDialog({ 
  ...
  const handleAdd = () => {
    ...
    const selectedFields = allFields.filter(f => selectedIds.has(f.id)).map(f => ({
      ...f,
      dataType: f.type, // Map type to dataType for consistency
      sourceMapping: "[System Computed]",
      category: "Derived",
      isPii: false
    }));
