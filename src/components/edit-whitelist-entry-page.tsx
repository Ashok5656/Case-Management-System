...
interface EditWhitelistEntryPageProps {
  initialData: {
    id: string;
    entityId: string;
    type: string;
...
export function EditWhitelistEntryPage({
  initialData,
  onBack,
  onBreadcrumbNavigate,
  breadcrumbs,
  parentTab
}: EditWhitelistEntryPageProps) {
  const isScenarioWise = parentTab?.includes("scenario") || initialData?.listType?.includes("scenario");
  
  const [entityId, setEntityId] = useState(initialData?.entityId || "WL-SW-001");
  const [entityType, setEntityType] = useState(initialData?.type || "BIN");
  const [entityValue, setEntityValue] = useState(initialData?.value || "411111");
  const [reason, setReason] = useState(initialData?.reason || "Trusted issuer BIN for cross-border transactions");
  
  const dateParts = initialData?.dateRange?.split(" -> ") || ["", ""];
  const [validFrom, setValidFrom] = useState(dateParts[0] || "01-01-2024 08:00");
  const [expiryDate, setExpiryDate] = useState(dateParts[1] || "31-12-2024 20:00");
  
  const [comments, setComments] = useState(initialData?.comments || "Used for high-value transaction monitoring");
  
  // Initialize with some default scenarios for existing records
  const [selectedScenarios, setSelectedScenarios] = useState<string[]>(
    isScenarioWise ? [AVAILABLE_SCENARIOS[0], AVAILABLE_SCENARIOS[2]] : []
  );
    ...
    <div className="flex flex-col h-full w-full bg-white relative font-['Inter']">
      ...
      <div className="flex-1 overflow-y-auto no-scrollbar [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none] p-8">
        <div className="w-full flex flex-col gap-8 max-w-full animate-in fade-in duration-500">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            <div className="space-y-2">
              ...
              <Input 
                value={entityId}
                readOnly
                className="h-[46px] border-[#E0E0E0] rounded-[8px] focus:ring-1 focus:ring-[#2A53A0] focus:border-[#2A53A0] text-[14px] bg-[#F4F4F4] shadow-none cursor-not-allowed"
              />
            </div>
            <div className="space-y-2">
              <label className="text-[14px] font-semibold text-[#161616]">
          ...
          {/* New Row: Scenarios (Strictly only Scenario wise entry) */}
          {isScenarioWise && (
            ...
            <div className="p-8 flex flex-col items-center text-center space-y-4">
              ...
              <div className="space-y-2">
                ...
                <p className="text-[14px] text-gray-500 leading-relaxed">
                  Whitelist entry <strong>"{entityId}"</strong> has been successfully updated.
                </p>
              </div>
